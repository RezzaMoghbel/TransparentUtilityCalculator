CREATE   PROCEDURE [utility].[rpt_MonthlyUsage_ByType]
    @UserId    NVARCHAR(450),
...
BEGIN
    ...
    -- Aggregate usage by month for the chosen type
    ...
    SELECT
        ...
        CONVERT(decimal(18,3), SUM(r.UnitsUsed))                           AS UnitsUsed
    INTO #agg
    ...
    -- Calendar of months so empty months show as 0
    ;WITH months AS (
    ...
    SELECT
        m.MonthStart,
        ISNULL(a.UnitsUsed, 0) AS UnitsUsed
    FROM months AS m
    LEFT JOIN #agg AS a
